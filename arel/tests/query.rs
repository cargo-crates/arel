...
mod query {
    ...
    fn test_select() {
        ...
        let sql = User::query().select(json!(["name", "age"])).distinct().to_sql().unwrap();
    ...
    fn test_where() {
        ...
        let sql = User::query().where_range("age", 18..).to_sql().unwrap();
        assert_eq!(sql, "SELECT `users`.* FROM `users` WHERE `users`.`age` >= 18");
        let sql = User::query().where_range("age", ..=25).to_sql().unwrap();
